...
pub fn render(frame: &mut Frame, app: &App) {
    ...
    .split(rows[1]);
    palette::render(frame, cols[0], app, app.focus == Pane::Palette);
    canvas::render(frame, cols[1], app, app.focus == Pane::Canvas);
    preview::render(frame, cols[2], app, app.focus == Pane::Preview);

    status_line(frame, rows[2]);
...
fn status_line(frame: &mut Frame, area: Rect) {
    ...
    let hints = Line::from("tab switch pane   ? help   q quit ").alignment(Alignment::Right);
...
mod tests {
    ...
    fn full_layout_with_a_loaded_pipe() {
        let app = App::with_pipe(
            Registry::with_builtins(),
            sample_pipe(),
            PathBuf::from("news_pipeline.pipe"),
        );
        insta::assert_snapshot!(render_to_string(&app, 120, 40));
    }

    #[test]
    fn too_small_notice() {
        let app = App::new(Registry::with_builtins());
        insta::assert_snapshot!(render_to_string(&app, 60, 15));
    }
