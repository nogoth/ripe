...
*splices* the new node into them.

1. `a` `f` — insert a **fetch_feed** source. `Enter` opens its params; type
   `https://hnrss.org/frontpage` into URL, then `Ctrl-S` to apply.
...
5. Press `3` (the filter), then `a` `r` — a **regex** node, spliced after
   it. `Enter`; set field `description`, `Tab`, pattern
   `Points:\s*(?<score>\d+)`, `Tab`, `Space` to flip mode to `extract`,
   `Ctrl-S`. Named capture groups become item fields — items now carry a
   `score`.
6. Press `4` (the regex), then `a` `t` — another **filter**, rule
   `score > 100`. Watch the item counts drop on the canvas.
7. `a` `f` — a second **fetch_feed**. Sources have no input, so it lands
   unwired as a new branch; set its URL to `https://lobste.rs/rss`.
8. Press `5` (the score filter), then `a` `u` — **union**, spliced before
   the output. Wire the branch in: press `6` (the second fetch), `c`,
   `7` (the union), `c`. Union's input is variadic — it accepts any number
   of incoming streams.
9. Press `7` (the union), then `a` `s` — **sort**; by `pubDate`, `Tab`,
   `Space` to flip the order to `desc`, `Ctrl-S`.
